...
//		$this->test();
	...
	{
		$excelFileName = iconv('utf-8','gbk',base_path().'/自动生成的表格/裁判用表（默认分页）' . date("Y年m月d日") . '.xlsx');
...
//			$objPageSetup->setFitToHeight(10);
		...
		$excelFileName = iconv('utf-8','gbk',base_path().'/自动生成的表格/裁判用表（默认分页）' . date("Y年m月d日") . '.xlsx');
		if ($pageBreak=='分组') {
			$excelFileName = iconv('utf-8','gbk',base_path().'/自动生成的表格/裁判用表（分组分页）' . date("Y年m月d日") . '.xlsx');
		} elseif ($pageBreak=='组别') {
			$excelFileName = iconv('utf-8','gbk',base_path().'/自动生成的表格/裁判用表（组别分页）' . date("Y年m月d日") . '.xlsx');
		}
